...
// Takes user input and populates input array with arguments
...
int processExec(char **inputArr, int *value, int *bg_processes){
	...
	// Check if no input and continue
	if(inputArr[0] == NULL){
	
		return cont_running;
	}	

	// cd command instructions
	...
	// status command instructions
		...
		*value = 0;
		return cont_running;
	// if command is exit; return 0 and quit shell.
